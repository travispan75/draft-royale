...
export default function Home() {
  ...
  const remaining = useMemo(() => {
    if (!snap?.turnDeadline) return 0
    return Math.max(0, Math.floor((snap.turnDeadline - (now + offset)) / 1000))
  ...
  const myRole =
    snap &&
    (snap.players.p1 === playerIdRef.current
      ? 'p1'
      : snap.players.p2 === playerIdRef.current
        ? 'p2'
        : null)

  const canPick =
    !!(snap && myRole && snap.status === 'draft' && snap.turn === myRole)
  ...
  const onPickAction = (card: string) => {
    ...
    socketRef.current.emit('draft_pick', {
      roomId: id as string,
      ...
      >
        ...
        >
          ...
          >
            {snap && (
              <DeckGrid
                cards={myRole === 'p1' ? snap.picks.p2 : snap.picks.p1}
              />
            )}
          </div>
        </div>
        ...
        >
          ...
          >
            {snap && (
              <DeckGrid
                cards={myRole === 'p1' ? snap.picks.p1 : snap.picks.p2}
              />
            )}
          </div>
        </div>
